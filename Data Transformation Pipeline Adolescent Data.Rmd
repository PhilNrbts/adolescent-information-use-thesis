---
title: "Data Transformation Pipeline Adolescent Data"
...
Distance: 
  ANOVA (meaningful difference in distance between participants)
  pairwise correlation-test (meaningful correlation between previous trial
  points and distance of exploration)
  ANOVA between groups (do either adults or adolescents tend to select more
  distant tiles)

Phases:

  length_exploration, length_accumulation, length_exploitation:
    - one-sample t-tests to establish that they exist in the first place
    - t-test between participants, if difference in length per phase is meaningful
    
  threshold_accumulation, threshold_exploitation
    - ANOVA between participants, if difference in threshold is meaningful
    - pairwise t-test if 
    - ANOVA between groups, if thresholds differ between participants
    
    - a multilevel model with random intercept per participant, group and threshold_phase?
    
  color_class for thresholds
    - Cochran's Q test between participants, if difference in threshold is meaningful
    - Mann-Whitney between groups if thresholds differ between participants

    - a multilevel model for ordinal data with random intercept per participant,
    group, and threshold_phase
  
social_info_use:
  -Exploration
    -2 sample t-test on ratio of no_value_social_information use
    -2 sample t-test on ratio of value_social_information use
    -2 sample t-test on mean of social_information_value
  ...
  -compare mean social_information_value between stages (multilevel?)
  
  -would it have been beneficial to follow social agent?
...
data <- data %>%
  ...
  mutate(color_class = case_when(
    ...
    points >= 0 & points < 20 ~ 1, # got rid of the 0 category, as not really necessary
    points >= 20 & points < 40 ~ 3,
    points >= 40 & points < 60 ~ 5,
    points >= 60 & points < 150 ~ 7,
    points >= 150 ~ 9
  )) %>%
  mutate(color_class = color_class / 2) %>%
...
# For threshold_accumulation_color_class and threshold_exploitation_color_class
summary_data <- summary_data %>%
  mutate(
    threshold_accumulation_color_class = case_when(
      threshold_accumulation < -60 ~ -7,
      threshold_accumulation >= -60 & threshold_accumulation < -40 ~ -5,
      threshold_accumulation >= -40 & threshold_accumulation < -20 ~ -3,
      threshold_accumulation >= -20 & threshold_accumulation < 0 ~ -1,
      threshold_accumulation >= 0 & threshold_accumulation < 20 ~ 1,
      threshold_accumulation >= 20 & threshold_accumulation < 40 ~ 3,
      threshold_accumulation >= 40 & threshold_accumulation < 60 ~ 5,
      threshold_accumulation >= 60 & threshold_accumulation < 150 ~ 7,
      threshold_accumulation >= 150 ~ 9
    ) / 2,
    threshold_exploitation_color_class = case_when(
      threshold_exploitation < -60 ~ -7,
      threshold_exploitation >= -60 & threshold_exploitation < -40 ~ -5,
      threshold_exploitation >= -40 & threshold_exploitation < -20 ~ -3,
      threshold_exploitation >= -20 & threshold_exploitation < 0 ~ -1,
      threshold_exploitation >= 0 & threshold_exploitation < 20 ~ 1,
      threshold_exploitation >= 20 & threshold_exploitation < 40 ~ 3,
      threshold_exploitation >= 40 & threshold_exploitation < 60 ~ 5,
      threshold_exploitation >= 60 & threshold_exploitation < 150 ~ 7,
      threshold_exploitation >= 150 ~ 9
    ) / 2
  )
...
# Test if unique_rounds all got length 25 and cell is numerical

```{r}
# Test if unique_rounds all got length 25 and cell is numerical
# Group by unique_rounds and count the unique number of trials
trial_counts <- data %>%
  group_by(unique_rounds) %>%
  summarize(num_trials = n_distinct(trial))

# Now filter or view those unique_rounds where count is not equal to 25
incomplete_rounds <- trial_counts %>%
  filter(num_trials != 25)

# Print the incomplete_rounds data
print(incomplete_rounds)

# Convert 'cell' column to numeric
data <- data %>%
  mutate(cell = as.numeric(cell))

# Now check per 'unique_rounds' if there are any non-numeric values (now NA)
non_numeric_cells <- data %>%
  group_by(unique_rounds) %>%
  summarize(num_non_numeric_cells = sum(is.na(cell)))

# Now filter or view those 'unique_rounds' where there are non-numeric cells
non_numeric_cells <- non_numeric_cells %>%
  filter(num_non_numeric_cells > 0)

# Print the non_numeric_cells data
print(non_numeric_cells)
...
Explore phases Importantly, I removed gems from thresholds earlier, since gem are not really informative.
